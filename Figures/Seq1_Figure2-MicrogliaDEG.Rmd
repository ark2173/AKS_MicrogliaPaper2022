...
Genes<-c("Ccl4","Fos","Cdk4","Spp1","Cx3cr1","Csf1r","Tmem119")
...
Genes<-c("Mki67","Ccl12","Cx3cr1","Lpl","Cd9")
toptable[toptable$p_val_adj>0.05,]$lab<-NA
toptable$p_val_adj <- -log10(toptable$p_val_adj)
toptable[toptable$p_val_adj>200,]$p_val_adj<-200
toptable[!(toptable$lab %in% Genes),]$lab<-NA
...
xlab<-bquote(~Log[2] ~ "fold change")
ylab<-bquote(~-Log[10] ~ italic(P))
Volcano2<-ggplot(toptable, aes(x = avg_log2FC, y = p_val_adj)) + th + geom_point(na.rm = TRUE,size=0.1,color=ColorsTreatment[4])+ labs(title = '3d BBBO v Sham')+xlab(xlab) + ylab(ylab) + geom_label_repel(aes(label=lab),size=3,label.padding = unit(0.1, "lines"))+theme(panel.grid = element_blank())
...
Right<-plot_grid(OntoPlot,Phase,Blank,nrow=3,rel_heights = c(2,1,1.5),labels = c("e","f","h"))
