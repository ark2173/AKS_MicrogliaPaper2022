...
Percent<-ggplot(tab,aes(x=Var2,y=Freq_Adj*100,fill=Var2,color=Var2))+stat_summary(fun="mean",geom="bar")+stat_summary(geom="errorbar",width=0.2,color="black")+scale_fill_manual(values=ColorsTreatment)+scale_color_manual(values=LinesTreatment)+theme(legend.position="none",
        plot.margin=margin(0.5,0.5,0.5,0.5,"cm"),
        panel.background = element_blank(),
        ...
        panel.grid = element_blank())+stat_compare_means(method="anova",label.y = 45,size=3) + scale_y_continuous(expand = c(0,0))+ylab("DAM (% of microglia)")
```
