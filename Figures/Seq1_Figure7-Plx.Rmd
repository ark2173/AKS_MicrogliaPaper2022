...
MRIs_Long$name<-recode(MRIs_Long$name,
                       ...
                       "Day.4.Norm"="Day 4",
                       "Day.5.Norm"="Day 5")
MRIs_Long$Group<-factor(MRIs_Long$Group,levels=c("Control","Repopulating","Depletion"))
A1<-ggplot(MRIs_Long,aes(x=name,y=value,color=Group))+stat_summary(geom="errorbar",position=position_dodge(width=0.75))+stat_compare_means(method="t.test",size=3,label="p.format",show.legend = FALSE)+stat_summary(fun="mean",geom="point",position=position_dodge(width=0.75))+scale_color_manual(values=CustomColors)+theme_bw()+ylab("Opening Volume (Relative to Day 0)")+xlab("Days after treatment")+theme(legend.position = c(0.85,0.8),legend.box.background = element_rect(color="black",linetype = "solid",fill="white"),legend.background = element_blank(),legend.title = element_blank())



MRIs<-MRIs[MRIs$Group!="Repopulating",]
MRIs_Long<-pivot_longer(MRIs,cols = c("Day.1","Day.2","Day.3","Day.4","Day.5"))
A2<-ggplot(MRIs_Long,aes(x=name,y=value,color=Group))+stat_summary(geom="errorbar",position=position_dodge(width=0.75))+stat_compare_means(method="t.test",size=3,label="p.format",show.legend = FALSE)+stat_summary(fun="mean",geom="point",position=position_dodge(width=0.75))+scale_color_manual(values=CustomColors)+theme_bw()+ylab("Opening Volume (Relative to Day 0)")+xlab("Days after treatment")+theme(legend.position = c(0.85,0.8),legend.box.background = element_rect(color="black",linetype = "solid",fill="white"),legend.background = element_blank(),legend.title = element_blank())
