...
Dim1<-DimPlot(CAM,group.by = "clusters_labeled",label=FALSE,cols = ColorMacrophageClusters)+theme(plot.title = element_blank(),
        ...
        legend.position = c(0.3,0.05))+guides(color=guide_legend(override.aes = list(size=4),nrow=1))+scale_y_continuous(expand=c(0,2))
```
...
levels(Treatment)<-c("Naive","Sham","24h","72h")
HM<-Heatmap(Data,
            cluster_rows = FALSE,
...
BC3<-ggplot(df, aes(y=Freq,x=Treatment,fill=Phase)) + 
  scale_fill_manual(values = rev(ColorPhase),guide=guide_legend(ncol=1)) + 
...
Inset<-ggdraw(Dim1 + ylim(-9.5,7.51)+theme_half_open(12) + theme(plot.title = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),legend.position = c(0,0.1))) +
  ...
  draw_plot_label(
    ...
    c(0.75),
    size = 12)
Top<-plot_grid(Inset,Pie,BC3,nrow=1,align = "h",axis = "t",labels = c("A","C"))
...
ggsave("Figure3.png",width = 8.5,height = 8.5,units = "in",bg="white")
```
