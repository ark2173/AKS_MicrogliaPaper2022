...
#FeaturePlot(Microglia,features = DAMMarkers)&theme(legend.position = "none")
DotPlot(Microglia,features = DAMMarkers)&theme(legend.position = "none")
```
...
DefaultAssay(Microglia)<-"RNA"
clustermarkers_1d <- FindMarkers(Microglia, ident.1 = "1d BBBO",ident.2 = "Sham",slot = "scale.data")
clustermarkers_3d <- FindMarkers(Microglia, ident.1 = "3d BBBO",ident.2 = "Sham",slot = "scale.data")
clustermarkers_AllTreatment <- FindAllMarkers(Microglia, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
```
...
```{r}
markersDAM<-FindMarkers(Microglia,ident.1 = "2")
setwd("F:/Data/Sequencing/SavedData")
save(markersDAM,Onto_3d,Onto_1d,clustermarkers_AllTreatment,clustermarkers_3d,clustermarkers_1d,Microglia,Data_New_Sub,clustermarkers,file="5-MicrogliaFigure2.RData")
```
...
DAMMarkers<-FindMarkers(Microglia,ident.1 = "2")
DAM<-subset(Microglia,idents="2")
Idents(DAM)<-"Treatment"
DAM_markers_treatment<-FindAllMarkers(DAM,only.pos = T)
DAM_markers_treatment$gene<-rownames(DAM_markers_treatment)
marks <- DAM_markers_treatment[DAM_markers_treatment$p_val_adj<0.05,]
marks_1d <- FindMarkers(DAM,ident.1 = "1d BBBO",ident.2 = "Sham")
marks_3d <- FindMarkers(DAM,ident.1 = "3d BBBO",ident.2 = "Sham")
marks_1d<-marks_1d[marks_1d$avg_log2FC>0,]
marks_1d<-marks_1d[marks_1d$p_val_adj<0.05,]
library(topGO)
DAM_Onto_1d<-GO_Search(DAM,marks_1d)
...
marks_3d<-marks_3d[marks_3d$avg_log2FC>0,]
marks_3d<-marks_3d[marks_3d$p_val_adj<0.05,]
DAM_Onto_3d<-GO_Search(DAM,marks_3d)
...
```{r}
setwd("F:/Data/Sequencing/SavedData")
save(DAMMarkers,DAM_Onto_1d,DAM_Onto_3d,DAM_markers_treatment,DAMMarkers,Onto_3d,Onto_1d,clustermarkers_AllTreatment,clustermarkers_3d,clustermarkers_1d,Microglia,clustermarkers,file="5-MicrogliaFigures.RData")
```
